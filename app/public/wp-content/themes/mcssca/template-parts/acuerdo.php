<div id="elemento_<?php echo get_the_ID() ?>" class="col">
   <div class="card mb-5 shadowcss" style="background: linear-gradient(to right, rgba(64, 154, 247, 1), rgba(43, 170, 177, 1)) !important; color: #fff;">
      <div class="card-header pt-4 d-flex">
         ...
         <div class="col-md-4">
            <h6><?php echo (get_post_meta(get_the_ID(), '_vigente', true)) ? mcssca_get_page_att(get_post_type())['status'] : 'Ejecutado el: ' . get_post_meta(get_the_ID(), '_f_seguimiento', true) ?></h6>
         </div>
      </div>
      <div class="card-body">
         <h5 class="card-title"> <a class="text-white" href="<?php echo get_the_permalink() . '?pag_ant=' . mcssca_get_page_att(get_post_type())['pag'] ?>"><?php echo get_post(get_post_parent(get_the_ID()))->post_title ?></a></h5>
